...
- `IWatchlistRepository` — CRUD da watchlist de jogadores (`tracked_players`): `GetActiveAsync` (read path quente dos jobs), `GetByPlayerIdAsync`, `UpsertAsync`, `DeactivateAsync` (soft-delete), `TouchLastCollectedAsync` e `QueryPagedAsync`. Ver [`docs/watchlist.md`](watchlist.md).
...
| `ExpectedNetMargin` | `TEXT` (decimal) | margem líquida após taxa (filtros na API) |
...
Índices: `ix_trade_opportunities_last_recomputed`, `ix_trade_opportunities_is_stale`, `ix_trade_opportunities_opportunity_id` (`OpportunityId`).

### `tracked_players`

Watchlist persistida. Substitui os arrays `Jobs:PriceCollection:Players` e `Jobs:OpportunityRecompute:Players` em `appsettings` como fonte de verdade.

| Coluna | Tipo | Notas |
| --- | --- | --- |
| `PlayerId` | `INTEGER` | PK — `eaId` do FUT.GG (ver `docs/worker.md`) |
| `DisplayName` | `TEXT(256)` | rótulo humano (logs + admin UI) |
| `Overall` | `INTEGER?` | 0-99, usado no scoring quando disponível |
| `Source` | `INTEGER` | enum `WatchlistSource` (`Seed=0`, `AppSettings=1`, `Api=2`) |
| `AddedAtUtc` | `TEXT` (UTC) | preservado entre upserts para trilha de auditoria |
| `LastCollectedAtUtc` | `TEXT?` (UTC) | atualizado ao final de cada tick do `price-collection` |
| `IsActive` | `INTEGER` (0/1) | `false` = soft-delete (row preservada, ignorada pelos jobs) |

Índices: `ix_tracked_players_is_active` (`IsActive`), `ix_tracked_players_source` (`Source`).
